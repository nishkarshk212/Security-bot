"""
Link message blocking module.
Handles deletion of messages containing URLs.
"""
...
async def block_link_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Block messages containing links based on group settings."""
    ...
    if not re.search(link_pattern, message.text):
        return
    ...
    if not settings.get('block_links', True):
        return
    ...
    if user_id:
        ...
        if user_id in exempt_users:
            ...
            if user_specific and not user_specific.get('block_links', True):
                return  # User has links allowed
    ...
    try:
        ...
        if user:
            ...
            logger.info(f"Link message deleted in {chat_id} | User {user.id}")
