"""
Forwarded message blocking module.
Handles deletion of forwarded messages.
"""
...
async def block_forwarded_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Block forwarded messages based on group settings."""
    ...
    # Check if forward blocking is enabled
    if not settings.get('block_forwards', True):
        return
    ...
    if user_id:
        ...
        if user_id in exempt_users:
            ...
            if user_specific and not user_specific.get('block_forwards', True):
                return  # User has forwards allowed
    ...
    try:
        ...
        if user:
            ...
            logger.info(f"Forwarded message deleted in {chat_id} | User {user.id}")
